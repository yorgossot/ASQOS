...
class Analytical():
    ...
    def obtain_lindblads(self):
        ...
        for lind_op in range(self.lind_op_number ):
            ...
            L_nonzeros_pos = []
            AffectedStates = []
            for i in  range(L_matrix.rows):
                for j in  range(L_matrix.cols):
    ...
    def obtain_gate_performance_symbolic(self):
        ...
        H_symbolic = [ sp.Symbol(f'H_{diag}') for diag in GSRange] 
        self.H_symbolic = H_symbolic
        
        self.gate_time_symbolic =  sp.Abs( np.pi  /(H_symbolic[3]+H_symbolic[0]-H_symbolic[1]-H_symbolic[2]) ) * sp.Symbol('tgr') #tgr : gate time ratio
        gate_time_symbolic = sp.Symbol('tgs')
        ...
        for lind_op in range(self.lind_op_number):
            for which in GSRange:   
                l_symbolic =  sp.Symbol(f'L_{lind_op}_{which}')     
                L_symbolic.append(l_symbolic)     
                loss_symbolic = l_symbolic* sp.conjugate(l_symbolic)
                eff_eff_hamiltonian_symbolic[which]  -= sp.I*loss_symbolic / 2
        ...
        # Start at ++
        init_state = sp.Matrix(init_state.data.toarray().reshape(2**n_qubits))
        # Apply initial adjustments
        for i in range(2):
            rot_val = sp.Symbol(f'r0_i') 
        ...
        for i in GSRange:
            p_success_symbolic += sp.Abs(init_state[i,0])**2 * sp.exp( - loss_factors_symbolic[i])
        self.p_success_symbolic = p_success_symbolic
        
        #PSuccess_symbolic = sp.Symbol('pss')

        pure_evolution_symbolic = []  
        for i in GSRange:
            evolution_symbolic = sp.exp(- sp.I * eff_eff_hamiltonian_symbolic[i]*gate_time_symbolic)  
            pure_evolution_symbolic.append( evolution_symbolic )

        evolution_symbolic_HL = sp.Matrix(np.diag(pure_evolution_symbolic))
        
        self.evolution_symbolic_HL = evolution_symbolic_HL

        evolution_symbols = [sp.Symbol(f'u0'),sp.Symbol(f'u1'),sp.Symbol(f'u1'),sp.Symbol(f'u2') ] 
        evolution_symbolic = sp.Matrix(np.diag(evolution_symbols))
        ...
        self.rotation_symbolic = sp.Symbol('rot')
    ...
    def obtain_gate_performance_hardware(self, hardware_dict):
        ...
        De0_val =  sp.Symbol('Delta_e') - hardware_dict[sp.Symbol('Delta_max')]
        
        # Contains parametric De0, thus not added to self.hardware_dict
        hw_subs_dict = {**hardware_dict, sp.Symbol('Delta_e0'):De0_val}
        ...
        for elem,var in enumerate(self.L_symbolic):
            ...
            if type(L_num) is type(sp.Symbol('x')): L_num = L_num.subs(hw_subs_dict)
        ...
        self.substitution_dict_hw[sp.Symbol(f'u0')] = self.hardware_evolution_unitary[0,0]
        self.substitution_dict_hw[sp.Symbol(f'u1')] = self.hardware_evolution_unitary[1,1]
        self.substitution_dict_hw[sp.Symbol(f'u2')] = self.hardware_evolution_unitary[3,3]
        
        self.rotation_hardware = self.rotation_symbolic_HL.subs(self.substitution_dict_hw)

        self.substitution_dict_hw[sp.Symbol('rot')] = self.rotation_hardware
    ...
    def tunable_performance(self, tunable_parameters_dict,opt_settings_dict):
        ...
        '''
        try:
            tunable_parameters_dict[sp.Symbol('Delta_E')] = tunable_parameters_dict[sp.Symbol('Delta_EC')] * tunable_parameters_dict[sp.Symbol('C')]
            HL_dict = self.substitution_dict_hw.copy()
            for key in HL_dict:
                if type(HL_dict[key]) is type(sp.Symbol('x')): HL_dict[key] = HL_dict[key].subs(tunable_parameters_dict)

            HL_dict = { **HL_dict ,**tunable_parameters_dict }


            gate_time = float(sp.re( self.gate_time_symbolic.subs( HL_dict )))
            HL_dict[sp.Symbol('tgs')] =  gate_time

            p_success = float(sp.re(self.p_success_symbolic.subs(HL_dict)))
            HL_dict[sp.Symbol('pss')]   = p_success
            ...
            for i in range(3):
                HL_dict[sp.Symbol(f'u{i}')] = HL_dict[sp.Symbol(f'u{i}')].subs(HL_dict)

            
            HL_dict[sp.Symbol('rot')] = self.rotation_hardware.subs(HL_dict)
            
            fid_expression = self.GHZ_fidelity_dict[opt_settings_dict["ghz_dim"]]['fidelity']
               
            fidelity =   float(sp.re(fid_expression.subs(HL_dict).subs( tgs=gate_time)))
    ...
    def optimize_gate_performance_hardware(self, parameter_bounds, opt_settings_dict ):
        ...
        parameters_to_optimize = [sp.Symbol('C'),sp.Symbol('Delta_e'),  sp.Symbol('Delta_EC'), sp.Symbol('tgr')]
        ...
        if opt_settings_dict["ghz_dim"] > 2:
            for i in range(opt_settings_dict["ghz_dim"]): parameters_to_optimize.append( sp.Symbol(f'r{i}_p') )
            for i in range(opt_settings_dict["ghz_dim"]): parameters_to_optimize.append( sp.Symbol(f'r{i}_i') )
            for i in range(opt_settings_dict["ghz_dim"]-2): parameters_to_optimize.append( sp.Symbol(f'r{i}_by') )
            for i in range(opt_settings_dict["ghz_dim"]-3): parameters_to_optimize.append( sp.Symbol(f'r{i}_bz') )
            ...
            parameters_to_optimize.append( sp.Symbol(f'r{0}_p') )
            parameters_to_optimize.append( sp.Symbol(f'r{0}_i') )
        ...
        def cost_function(params):
            ...
            for i,ind in enumerate(self.post_rotations_range):
                tunable_parameters_dict[sp.Symbol(f'r{i}_p')] = params[ind]
            

            for i,ind in enumerate(self.initial_rotations_range):
                tunable_parameters_dict[sp.Symbol(f'r{i}_i')] = params[ind]


            for i,ind in enumerate(self.between_rotations_y_range):
                tunable_parameters_dict[sp.Symbol(f'r{i}_by')] = params[ind]

            for i,ind in enumerate(self.between_rotations_z_range):
                tunable_parameters_dict[sp.Symbol(f'r{i}_bz')] = params[ind]
        ...
        self.opt_tunable_dict[sp.Symbol('Delta_E')] = self.opt_tunable_dict[sp.Symbol('Delta_EC')] * self.opt_tunable_dict[sp.Symbol('C')]
        self.opt_tunable_dict[sp.Symbol('tgs')] = self.optimized_performance_dict['gate_time']
        self.opt_tunable_dict[sp.Symbol('rot')] = sp.re(self.rotation_hardware.subs(self.opt_tunable_dict))
    ...
    def GHZ_obtain_symbolic_fidelity_from_evolution(self,ghz_dims):
        ...
        if self.SimulationClass.load_analytical:
            ...
            for ghz_dim in ghz_dims:
                print(f'Obtaining and NOT saving expression for fidelity of GHZ-{ghz_dim}')
                n_qubits = ghz_dim
                GHZ_state = np.zeros(2**n_qubits)
                GHZ_state[[0,-1]] = 1/np.sqrt(2)
                GHZ_state_vec = sp.Matrix(GHZ_state)

                if self.symmetric:
                    evolution_symbols = [sp.Symbol(f'u0'),sp.Symbol(f'u1'),sp.Symbol(f'u1'),sp.Symbol(f'u2') ] 
                else:
                    evolution_symbols = [sp.Symbol(f'u{i}') for i in range(self.GSRange) ] 
                evolution_symbolic = sp.Matrix(np.diag(evolution_symbols))

                rotation_symbolic = sp.Symbol('rot')
                ...
                if ghz_dim==2:
                    for i in range(ghz_dim): 
                        rot_val = (sp.Symbol(f'r0_p') + rotation_symbolic )
                        R_post.append(gate_simulation_functions.R_z(rot_val))
                else:
                    for i in range(ghz_dim): 
                        rot_val = (sp.Symbol(f'r{i}_p') + rotation_symbolic )
                        R_post.append(gate_simulation_functions.R_z(rot_val))
                ...
                for i in range(ghz_dim-2): 
                    if i== ghz_dim-3:
                        ...
                        rot_val_y = sp.Symbol(f'r{i}_by') 
                        Ry = ten_r_p( gate_simulation_functions.R_y(rot_val_y),0)
                        R_bet.append(Ry)
                    else:   
                        rot_val_y = sp.Symbol(f'r{i}_by') 
                        Ry = ten_r_p( gate_simulation_functions.R_y(rot_val_y),0)
                        rot_val_z = sp.Symbol(f'r{i}_bz') 
                        Rz = ten_r_p( gate_simulation_functions.R_z(rot_val_z),0)
                        R_bet.append(Ry*Rz)

                    

                H = sp.Matrix( qt.qip.operations.hadamard_transform(1).data.toarray() )
                
                plus_state = qt.Qobj(np.array([1,1])/np.sqrt(2) )
                init_state = qt.tensor(*(plus_state for i in range(n_qubits)))

                # Start at ++
                current_state = sp.Matrix(init_state.data.toarray().reshape(2**n_qubits))
                ...
                aux_dict[sp.Symbol('f0')] = current_state[0,0].copy()
                aux_dict[sp.Symbol('f1')] = current_state[-1,0].copy()

                current_state[0,0]  = sp.Symbol('f0',domain="complex")
                current_state[-1,0] = sp.Symbol('f1',domain="complex")

                # normalize state
                current_state = current_state/  sp.Matrix(current_state).norm()

                fidelity_sg =   sp.Abs(current_state.dot( GHZ_state_vec) )
