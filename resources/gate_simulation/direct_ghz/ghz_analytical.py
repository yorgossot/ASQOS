...
class Analytical():
    ...
    def obtain_gate_performance_symbolic(self):
        ...
        init_state = np.array([1 for i in GSRange])
        init_state = init_state/np.sqrt(np.sum(np.square(init_state)))
        ...
        self.evolution_unitary_symbolic = evolution_unitary_symbolic
        # Post process rotations
        rotation = {}
        rotation[0] = - gate_time_symbolic * (H_symbolic[0] - H_symbolic[2]) 
        rotation[1] = - gate_time_symbolic * (H_symbolic[0] - H_symbolic[4]) 

        # The following expressions are not fully expressed in terms of detunings necessarily.
        self.fidelity_ghz_symbolic = gate_simulation_functions.GHZ_symbolic_fidelity_from_evolution(evolution_unitary_symbolic,rotation) 
    ...
    def obtain_gate_performance_hardware(self, hardware_dict):
        ...
        De01_val =  sg.var('De1') - hardware_dict['max_split']
        De02_val =  sg.var('De2') - hardware_dict['max_split']

        CcDe0_dict = {sg.var('C'): hardware_dict['C'] , sg.var('c'): hardware_dict['c'] 
                    ,sg.var('De01'): De01_val,sg.var('De02'): De02_val }
    ...
    def tunable_performance(self, tunable_parameters_dict,opt_settings_dict):
        ...
        try:
            ...
            fidelity = float(sg.real(self.fidelity_ghz_symbolic.subs( HL_dict)))
    ...
    def optimize_gate_performance_hardware(self, parameter_bounds, opt_settings_dict ):
        ...
        parameters_to_optimize = [ sg.var('De1'),sg.var('De2'),  sg.var('DE'), sg.var('tgr')]
        # Add rotation parameters depending on the ghz_dim
        for i in range(2): parameters_to_optimize.append( sg.var(f'r{i}_r') ) 
        ...
        def cost_function(params):
            ...
            De1_val,De2_val, DE_val, tg_ratio  = params[0:4]
            tunable_parameters_dict = {sg.var('De1') : De1_val, sg.var('De2'):De2_val , sg.var('DE') : DE_val , sg.var('tgr'): tg_ratio}
            
            for i in range(2):
                tunable_parameters_dict[sg.var(f'r{i}_r')] = params[3+i]
        ...
        self.opt_tunable_dict = {parameters_to_optimize[i]: result.x[i] for i in range(len(result.x))}
