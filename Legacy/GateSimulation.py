...
class Analytical():
    ...
    def obtain_gate_performance_hardware(self, C_val , c_val, max_split):
        ...
        PureEvolutionUnitary = []
        PureEvolutionVector = [] 
        pure_evolution_unitary_symbolic = []  
        for i in GSRange:
            Evolution = sg.exp(EffEffHamiltonian[i]*gate_time_symbolic)  
            PureEvolutionUnitary.append( Evolution )
            PureEvolutionVector.append( Evolution*init_state[i] )
        ...
        PureEvolutionUnitary = np.diag(PureEvolutionUnitary)
        PureEvolutionUnitarySg = sg.Matrix(sg.SR, PureEvolutionUnitary) / sg.sqrt(PSuccess_symbolic)
        ...
        # Post process rotations
        rot_r = sg.var('rot_r')
        r1 = - gate_time_symbolic * (H[0] - H[2]) * rot_r
        self.r1 = - gate_time_symbolic * (H[0] - H[2])   
        dim = self.setup.gs_dim
        R1 = sg.Matrix(sg.SR,np.zeros((dim,dim)))
        R1_list = [sg.exp(sg.I*r1*i) for i in range(2) for j in range(2) ]
        for i in range(dim):
        ...
        r2 = -gate_time_symbolic * (H[0] - H[1]) * rot_r
        self.r2 = -gate_time_symbolic * (H[0] - H[1])
        R2 = sg.Matrix(sg.SR,np.zeros((dim,dim)))
        R2_list = [sg.exp(sg.I*r2*j) for i in range(2) for j in range(2) ]
        for i in range(dim):    
            R2[i,i] = R2_list[i]
        ...
        unitary_with_rotations_sym = R2*R1*PureEvolutionUnitarySg 
        ...
        self.fidelity_hw1       = sg.abs_symbolic(( unitary_with_rotations_symbolic*sg.vector(init_state) ) *FinalVector)
        self.fidelity_ghz3_hw   = GHZ_3_symbolic_fidelity_from_evolution(unitary_with_rotations_symbolic) # Contains tgs and pss
        self.p_success_hw       = PSuccess                                        # Contains tgs 
        self.gate_time_hw       = gate_time

        self.substitution_dict = {}
        for i in GSRange:
            self.substitution_dict[ H_symbolic[i] ] = H[i]
        for lind_op_which in range(self.lind_op_number*4):
            self.substitution_dict[ L_symbolic[lind_op_which] ] = L[lind_op_which]
